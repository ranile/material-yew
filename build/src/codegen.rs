use crate::{Component, Property, utils};
...
pub fn gen_component(component_name: &str, mut variants: Vec<Component>) -> String {
    if variants.len() == 1 {
        ...
        let props = gen_properties(&component, true).to_string();
        let to_html = gen_to_html(&component);

        let module_dir = format!("/build/dist/{}.js", component_name);
...
pub fn {component_name}(props: &Props) -> Html {{
    ...
    {to_html}
        ...
        let (props, variants_enum) = {
            ...
            let props = utils::format_tokens(gen_properties(&component, true).to_string()).unwrap();
            (props, variants_enum)
        };
